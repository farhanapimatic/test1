...
```go
func (me *EVENTS_IMPL) GetEventsCollectionByStoryId(input *GetEventsCollectionByStoryIdInput, queryParameters map[string]interface{})(*models_pkg.EventDataWrapperModel,error)
```
...
```go
collect := new (events_pkg.GetEventsCollectionByStoryIdInput)
...
result,_ = events.GetEventsCollectionByStoryId(collect, queryParams, )
